...
def _save_checkpoint(model, optimizer, scheduler, epoch, history, path,
...
def train_fold(
    ...
    if resume and os.path.exists(ckpt_path):
        ...
        start_epoch += 1
        print(f'[TRAIN] Resumed from epoch {start_epoch - 1} (best_loss={best_loss:.4f}, patience={patience_counter})')
    ...
    print(f'[TRAIN] {fold_tag} | {device} | {target_name} | train={n_train} val={n_val} | batch={batch_size} ({n_batches} batches/epoch)')
    ...
    for epoch in range(start_epoch, config['epochs'] + 1):
        ...
        classical_norm = float(np.mean([v for k, v in grad_norms.items() if 'q_weights' not in k] or [0]))
        quantum_norm = float(np.mean([v for k, v in grad_norms.items() if 'q_weights' in k] or [0]))
        ...
        val_metrics = evaluate_all(preds_arr, targets_arr) if len(targets_arr) > 0 else {
            'mse': 999.0, 'mae': 999.0, 'r2': 0.0, 'mape': 999.0, 'var_ratio': 0.0
        }
        ...
        if epoch % 10 == 0:
            _save_checkpoint(model, optimizer, scheduler, epoch, history, ckpt_path,
                             best_loss, patience_counter)
    ...
    if best_model_wts:
        ...
        torch.save(model.state_dict(), os.path.join(run_dir, f'best_model_{fold_tag}.pt'))
...
def train_model(config: dict):
    ...
    raw_path = config.get('raw_data_path', os.path.join('data', 'raw', '2dmatpedia_full.json'))
    ...
    # Initialize extractor (normalization stats computed per-fold to prevent leakage)
    ...
    if k > 1:
        ...
        for i, (train_mats, val_mats) in enumerate(folds):
            ...
            train_graphs = prepare_pyg_dataset(train_mats, config['target_name'], extractor, config['cutoff'])
            print('[DATA] Pre-computing val graphs...')
            val_graphs = prepare_pyg_dataset(val_mats, config['target_name'], extractor, config['cutoff'])

            # Compute strictly on training data to prevent leakage
            p_mean, p_std = compute_normalization_stats(train_mats, config['target_name'], extractor)
            
            metrics = train_fold(config, train_graphs, val_graphs, p_mean, p_std,
                                 run_dir, fold=i+1, resume=config.get('resume', False))
        ...
        with open(os.path.join(run_dir, 'kfold_summary.json'), 'w') as f:
            json.dump({k: {'mean': float(np.mean([m[k] for m in all_metrics])),
                          'std': float(np.std([m[k] for m in all_metrics]))}
                      for k in all_metrics[0]}, f, indent=2)
    else:
        ...
        # Compute strictly on training data
        p_mean, p_std = compute_normalization_stats(train_mats, config['target_name'], extractor)
        
        train_fold(config, train_graphs, val_graphs, p_mean, p_std,
                   run_dir, resume=config.get('resume', False))
