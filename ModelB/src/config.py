...
def get_default_config() -> Dict:
    return {
        ...
        # Quantum — conservative baseline within Cerezo BP bound
        'n_qubits': 8,            # 2^8=256 Hilbert space; safe per McClean 2018
        'n_vqc_layers': 3,        # = log2(8) per Cerezo 2021 local-cost bound
        'quantum_backend': 'lightning_gpu',  # lightning_gpu|lightning_cpu|simulator
        'diff_method': 'adjoint',            # auto-overridden in model.py per backend
        ...
        'filter_elements': None,  # List[str] — keep only materials containing ALL listed elements
...
def parse_args() -> Dict:
    ...
    p = argparse.ArgumentParser(description='Q2DM_v2 — Hybrid Graph-QNN Training')
    ...
    p.add_argument('--quantum_backend', type=str, default=config['quantum_backend'],
                   choices=['simulator', 'lightning_cpu', 'lightning_gpu', 'ibm', 'ionq'])
    p.add_argument('--diff_method', type=str, default=config['diff_method'],
    ...
    p.add_argument('--filter_elements', type=str, default=None,
    ...
    p.add_argument('--schnet_interactions', type=int, default=config['schnet_interactions'])
    p.add_argument('--n_features_compressed', type=int, default=config['n_features_compressed'])
    ...
    if config.get('filter_elements') and isinstance(config['filter_elements'], str):
        config['filter_elements'] = [e.strip() for e in config['filter_elements'].split(',')]
    return config
