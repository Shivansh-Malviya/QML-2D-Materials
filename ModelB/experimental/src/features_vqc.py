...
# ---------------------------------------------------------------------------
# CORRELATED_TARGETS ΓÇö scientifically validated exclusion map
#
...
#     ΓåÆ These are the SAME quantity computed with/without vdW functional.
#       Pearson r Γëê 0.99+. Must mutually exclude.
#
#   exfoliation_energy_per_atom : E_exf = E_2D - E_bulk (per atom)
#     ΓåÆ Directly computed FROM energy_per_atom.
...
#     ΓåÆ Computed from energy_per_atom vs hull of competing phases.
#       Weakly correlated with energy_per_atom (r Γëê 0.3-0.5 across 2Dmatpedia).
...
#     ΓåÆ magnetism is a categorical label derived from total_magnetization.
#       Must mutually exclude.
#
#   bandgap : No direct mathematical derivation from other fields.
#     ΓåÆ Independent property. No exclusions needed beyond itself.
# ---------------------------------------------------------------------------
CORRELATED_TARGETS = {
    "bandgap": set(),
    ...
    "total_magnetization": {"magnetism"},
    "decomposition_energy": {"energy_per_atom", "energy_vdw_per_atom"},
}
...
FEATURE_REGISTRY = [
    ...
    ("total_magnetization", "Total magnetic moment from DFT (╬╝_B)"),
    # Symmetry & composition (2 features from metadata)
    ("nelements", "Number of unique chemical elements in the material"),
    ("sg_number", "Space group number (1ΓÇô230), encodes crystal symmetry"),
    # Structural (6 features, computed from structure)
    ("coord_num", "Mean coordination number (neighbors within 3.0 ├à)"),
    ("packing_density", "Number of atoms / unit cell volume (atoms/┼│)"),
    ("bond_density", "Number of bonds / unit cell volume (r < 2.8 ├à)"),
    ("layer_thickness", "z_max - z_min of atomic positions (├à)"),
    ("mean_electronegativity", "Mean Pauling electronegativity of all atoms"),
    ("interlayer_spacing", "Max gap in sorted z-coordinates (├à)"),
...
def _get_electronegativity(element_symbol: str) -> float:
    """Get Pauling electronegativity. Chain: pymatgen ΓåÆ mendeleev ΓåÆ hardcoded fallback."""
    if _HAS_PYMATGEN:
        try:
