...
class ModelB(nn.Module):
    ...
    def __init__(self, config: Dict, phys_mean: torch.Tensor, phys_std: torch.Tensor):
        ...
        # Quantum device ΓÇö fallback chain: requested ΓåÆ lightning.qubit ΓåÆ default.qubit
        import warnings
        ...
        for _bn, _dm in _FALLBACK_CHAIN:
            try:
                ...
                if _bn != backend_name:
                    warnings.warn(
                        f"[MODEL] Backend fallback: {backend_name!r} ΓåÆ {_bn!r}"
                    )
    ...
    def forward(self, batch) -> torch.Tensor:
        ...
        # ΓöÇΓöÇ Batched SchNet ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Batched physics features ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        p_norm = (batch.physics - self.phys_mean) / self.phys_std  # [B, n_phys]

        # ΓöÇΓöÇ Batched compress + project ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ VQC Execution ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        # Parameter broadcasting is supported by default.qubit + backprop.
        # It is NOT supported by lightning.gpu or lightning.qubit + adjoint.
        backend_name = getattr(self.dev, "short_name", "")

        if backend_name == "default.qubit":
            # Batched tensor math (eliminates Python loop)
            q_raw = self._qnode(q_inputs.T, self.q_weights)  # [n_qubits, B]
            q_out = torch.stack(q_raw).T.float()  # [B, n_qubits]
        else:
            # Per-sample VQC (PennyLane fast simulators cannot batch circuits)
            B = q_inputs.shape[0]
            q_outputs = []
            for i in range(B):
                q_out_i = torch.stack(self._qnode(q_inputs[i], self.q_weights))
                q_outputs.append(q_out_i)
            q_out = torch.stack(q_outputs).float()  # [B, n_qubits]

        # ΓöÇΓöÇ Batched readout ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
